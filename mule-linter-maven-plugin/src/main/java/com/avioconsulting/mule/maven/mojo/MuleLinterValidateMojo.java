...
public class MuleLinterValidateMojo extends AbstractMuleLinterMojo {
    ...
    @Parameter(property = "ruleConfiguration", defaultValue = "${basedir}/muleLinter.groovy" ,readonly = false, required = false)
    private File ruleConfiguration;

    @Parameter(property = "appDir", defaultValue = "${basedir}" ,readonly = false, required = false)
    private File appDir;

    @Parameter(property = "format", defaultValue = "CONSOLE,JSON", readonly = false, required = false)
    private List<FormatOptionsEnum> formats;

    @Parameter(property = "failBuild", readonly = false, required = false, defaultValue = "false")
